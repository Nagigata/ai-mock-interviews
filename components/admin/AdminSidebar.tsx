...
const AdminSidebar = ({ collapsed, onToggle }: AdminSidebarProps) => {
  ...
  return (
    <aside className="fixed bottom-0 left-0 top-16 z-40 flex w-[var(--admin-sidebar-width)] flex-col border-r border-foreground/10 bg-background/95 backdrop-blur-md transition-[width] duration-300 ease-out">
      <div
        className={cn(
          "flex items-center border-b border-foreground/10 p-5",
      ...
      >
        {!collapsed && (
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">System Management</p>
          </div>
        )}
        <button
          type="button"
      ...
      <div className="border-t border-foreground/10 p-3">
        <Link
          ...
          className={cn(
            "flex items-center rounded-xl py-2.5 text-sm text-muted-foreground transition-colors hover:bg-foreground/5 hover:text-white",
